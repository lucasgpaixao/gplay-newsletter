...
**Marcação de foco (evento)**:
Indicação editorial de que o **evento** pode ser o **evento em foco** na **capa**; no máximo um **evento** marcado por vez entre os ainda não encerrados.
_Evit_: vários eventos marcados simultaneamente; confundir com **destaque na capa** (bloco de vídeos).

**Evento em foco**:
Único **evento** em hero na **capa** (countdown + amostra de cobertura); escolhido entre os com **marcação de foco (evento)**; se mais de um estiver marcado por engano, desempate pelo **início** mais próximo no tempo.
_Evit_: três cards de evento com peso igual na **capa**; escolha automática ignorando a marcação editorial.
...
_Evit_: cron agressivo o mês inteiro; acelerar para todos os **eventos** em paralelo; **coleta acelerada** sem **marcação de foco (evento)**.
...
- A **capa** exibe no máximo um **evento em foco** quando houver candidato válido (marcado, não **encerrado**); senão, **teaser de evento (capa)** se houver **início** em até sete dias; senão, só **edição do dia** (manchetes, **destaque na capa** de **vídeos**, demais blocos).
...
> **Dev:** "Copa e Xbox Showcase estão com marcação de foco. Qual vai na capa?"
> **Editor:** "Só um **evento em foco** — desempate pelo **início** mais próximo. O outro fica em `/eventos` até virar a vez. Corrigimos o markdown: uma **marcação de foco (evento)** por vez."
...
- (resolvido) **Evento em foco na capa**: **marcação de foco (evento)** manual (D); desempate por **início** mais próximo (C) se houver mais de um marcado.
...
- (resolvido) **Marcação de foco (evento)** no acervo: apenas Xbox Showcase com foco ativo; demais `destaque: false` (ajustar no `.md` ao trocar o evento da vez).
