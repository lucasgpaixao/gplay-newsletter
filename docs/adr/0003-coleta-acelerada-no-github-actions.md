...
Adicionamos o workflow `coletar-acelerada.yml` (cron `*/20` UTC) que só executa `npm run coletar` quando `scripts/deve-coletar-acelerada.ts` confirma o **evento em foco** (próximo não encerrado) ao vivo entre **início** e **fim**. A lógica espelha `src/lib/eventos-core.ts` (sem Astro). O workflow horário permanece; ambos usam `concurrency.group: coletar` para serializar commits.
